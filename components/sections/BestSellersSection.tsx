...
import { Heart, ShoppingCart, Star, Loader2, ChevronRight, TrendingUp } from "lucide-react";
...
function BestSellerCard({ product, rank }: { product: Product; rank: number }) {
  ...
  return (
    ...
    >
      {/* Rank Badge */}
      <div className="absolute left-1.5 top-1.5 z-20 flex h-6 w-6 items-center justify-center rounded-full bg-gradient-to-br from-amber-400 to-amber-600 text-xs font-bold text-white shadow-md md:h-7 md:w-7 md:text-sm">
        {rank}
      </div>
      ...
      <div className="relative bg-[#FAFAFA]">
        {/* Discount badge */}
        {discount > 0 && (
          <span className="absolute left-8 top-1.5 z-10 rounded bg-primary px-1.5 py-0.5 text-[8px] font-bold text-white md:left-10 md:top-2 md:text-[9px]">
            -{discount}%
          </span>
        )}
        {/* Wishlist */}
        ...
        {rating > 0 && (
          <div className="mt-1 flex items-center gap-0.5">
            ...
            {product.soldCount && product.soldCount > 0 && (
              <span className="ml-1 text-[8px] text-muted-foreground md:text-[9px]">
...
export default function BestSellersSection({ products, title = "Best Sellers" }: BestSellersSectionProps) {
  ...
  return (
    <section className="bg-gradient-to-r from-amber-50 to-orange-50 py-4 md:py-6">
      <div className="mx-auto max-w-7xl px-3 md:px-4">
        {/* Header */}
        <div className="mb-3 flex items-center justify-between gap-2 md:mb-4">
          <div className="flex items-center gap-2">
            <div className="flex h-6 w-6 items-center justify-center rounded-full bg-gradient-to-br from-amber-400 to-amber-600 md:h-7 md:w-7">
